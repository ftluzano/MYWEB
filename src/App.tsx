...
import { Instagram, Sparkles, ArrowDown } from 'lucide-react';
        ...
        if (cached) {
          ...
          if (Array.isArray(parsed)) {
            return parsed.filter(
              (p) => p && p.username && p.username.toLowerCase() !== 'exclusive.kyle777'
            );
          }
        }
      } catch (e) {
      ...
      .then((json) => {
        if (json.success && Array.isArray(json.data)) {
          const filtered = json.data.filter(
            (p: InstagramProfileData) =>
              p && p.username && p.username.toLowerCase() !== 'exclusive.kyle777'
          );
          setSavedProfiles(filtered);
          try {
            localStorage.setItem('user_saved_instagram_profiles', JSON.stringify(filtered));
          } catch (e) {}
        }
      })
      .catch((err) => {
      ...
      <main className="relative z-10 w-full max-w-6xl mx-auto px-4 sm:px-6 flex-1 flex flex-col justify-center items-center py-6 sm:py-8">
        ...
        <div className="mt-8 flex flex-col items-center">
          ...
          >
            ...
            <div className="p-2 rounded-xl bg-gradient-to-tr from-[#f09433] via-[#dc2743] to-[#bc1888] text-white shadow-[0_0_15px_rgba(220,39,67,0.6)] group-hover:scale-110 transition-transform">
              <Instagram className="w-5 h-5" />
            </div>

            <span className="font-gothic text-sm sm:text-base font-bold tracking-[0.14em] text-zinc-100 group-hover:text-white uppercase">
              DROP INSTAGRAM HERE
            </span>

            {savedProfiles.length > 0 && (
              <span className="px-2 py-0.5 rounded-full bg-red-900/60 border border-red-700/60 text-xs font-mono font-bold text-pink-200">
                {savedProfiles.length}
              </span>
            )}

            <div className="flex items-center gap-1 text-pink-400 group-hover:text-pink-300">
