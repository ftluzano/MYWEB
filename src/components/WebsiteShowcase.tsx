...
export const WebsiteShowcase: React.FC<WebsiteShowcaseProps> = ({ projects }) => {
  ...
  return (
    <section id="featured-systems" className="w-full max-w-4xl mx-auto py-1">
      ...
      <div className="flex items-center justify-start sm:justify-center overflow-x-auto pb-2.5 mb-3.5 no-scrollbar gap-2 px-1">
        {projects.map((p, idx) => {
          ...
          return (
            <button
              ...
              className={`flex items-center gap-2 px-3 py-2 rounded-xl text-xs font-medium transition-all duration-200 cursor-pointer shrink-0 border ${
                isSelected
            ...
            >
              <div className={`p-1 rounded-md ${isSelected ? 'bg-red-950/90 text-red-400' : 'bg-zinc-900 text-zinc-500'}`}>
                {getProjectIcon(p.id, 'w-3 h-3')}
              </div>
              <div className="flex flex-col items-start text-left">
                <span className="text-[9px] font-mono text-zinc-500 leading-none">0{idx + 1}</span>
                <span className="tracking-wide text-xs font-semibold leading-tight">{p.title}</span>
              </div>
            </button>
          );
        })}
      ...
      <div className="bg-[#0b0b11]/95 border border-zinc-800/80 rounded-2xl shadow-[0_15px_40px_rgba(0,0,0,0.85)] overflow-hidden">
        ...
        <div className="bg-[#0f0f18] px-4 sm:px-6 py-2.5 border-b border-zinc-800/80 flex flex-wrap items-center justify-between gap-2.5">
          {/* Status & Identity */}
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-[#14141e] hover:bg-[#1a1a26] border border-zinc-800 text-xs font-mono text-zinc-300 hover:text-white transition-colors cursor-pointer"
              title="Copy URL"
            >
              {copied ? (
                <>
            ...
            <button
              ...
              className="flex items-center px-2 py-1 rounded-lg bg-[#14141e] hover:bg-[#1a1a26] border border-zinc-800 text-xs font-mono text-zinc-300 hover:text-white transition-colors cursor-pointer"
              title="Reload live screen"
            >
              <RefreshCw className={`w-3 h-3 text-zinc-400 ${isLoading ? 'animate-spin text-red-400' : ''}`} />
            </button>
            ...
            <a
              ...
              className="flex items-center gap-1.5 px-3 py-1 rounded-lg bg-red-950/90 hover:bg-red-900 border border-red-800/90 text-white text-xs font-medium tracking-wide shadow-[0_0_12px_rgba(220,38,38,0.35)] transition-all cursor-pointer font-sans"
              title="Open site in new tab"
            >
              <span className="text-[11px] font-semibold">Launch Site</span>
        ...
        <div className="p-3.5 sm:p-5 space-y-3">
          ...
          <div className="flex items-center justify-center gap-2.5 py-0.5">
            ...
            <h2 className="text-xl sm:text-2xl font-bold font-gothic tracking-wider text-zinc-100 text-center uppercase">
              {current.title}
            </h2>
          </div>

          {/* Interactive Browser Window (Clean, Fixed Sizing) */}
          <div className="rounded-xl overflow-hidden border border-zinc-800/90 bg-[#07070b] shadow-[0_10px_30px_rgba(0,0,0,0.8)]">
            {/* Window Chrome Header Bar */}
            <div className="bg-[#12121c] px-3.5 py-2 flex items-center justify-between border-b border-zinc-800">
              ...
              <a
                ...
                rel="noopener noreferrer"
                onClick={() => gothicAudio.playWarriorLinkSound()}
                className="group flex items-center gap-1.5 px-3 py-1 rounded-md bg-[#0a0a0f] border border-zinc-800/90 hover:border-zinc-700 max-w-md w-full mx-3 transition-colors cursor-pointer"
            ...
            {/* Live Screen Frame Viewport (Reduced, Fixed Compact Height) */}
            <div className="relative w-full h-[350px] sm:h-[390px] bg-[#050508]">
              {/* Sleek Loading Overlay while the website initializes */}
              {isLoading && (
                <div className="absolute inset-0 z-10 bg-[#07070b]/90 backdrop-blur-sm flex flex-col items-center justify-center gap-2.5 text-zinc-400">
                ...
                {projects.map((_, i) => (
                  <button
                    ...
                    className={`h-1.5 rounded-full transition-all cursor-pointer ${
                      i === currentIndex ? 'bg-red-500 w-5' : 'bg-zinc-700 hover:bg-zinc-500 w-1.5'
                    }`}
